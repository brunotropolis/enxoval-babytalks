"""
Gera 3 variações da Alline Vieira em cenário Baby Talks unificado.
"""
import os
...
REF = Path("D:/CLAUDE/baby-talks-prep/images/palestrante-alline.jpg")

PROMPT = """Generate a new professional editorial portrait of THE EXACT SAME WOMAN shown in the reference photo. It is CRITICAL that every facial feature is preserved with 100% accuracy: the exact same face shape (oval), the same eyes (light hazel-brown, warm and wide with defined dark eyebrows arched softly), the same nose, the same lips and wide bright smile showing white teeth, the same jawline, the same fair rosy skin tone, and the same hairstyle (long wavy blonde hair with balayage — brighter blonde on the ends fading to slightly darker natural blonde roots, natural center part, falling past shoulders). Same age (early 30s). Do NOT reinterpret or "beautify" — this must look like an actual photograph of THIS specific woman, indistinguishable from a professional shoot on the same day.
